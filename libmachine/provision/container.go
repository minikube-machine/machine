...
func init() {
	Register("Container", &RegisteredProvisioner{
		New: NewContainerProvisioner,
	})
}

// ContainerProvisioner is used in the kic scenario.
// The underlying Provisioner is an ubuntu SystemdProvisioner, since
// the container used by minikube is an ubuntu-based contaienr
...
func NewContainerProvisioner(d drivers.Driver) Provisioner {
	return &ContainerProvisioner{
		BuildrootProvisioner{
			NewSystemdProvisionerNOPKG("ubuntu", d),
		},
	}
}
...
func (p *ContainerProvisioner) GenerateDockerOptions(dockerPort int) (*DockerOptions, error) {
